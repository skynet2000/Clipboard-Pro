...
<img src="Icon.icns" width="128" height="128" alt="MClipboard Icon" />
...
[![架构](https://img.shields.io/badge/架构-Intel%20%7C%20Apple%20Silicon-lightgrey)]()
...
- ⌨️ **全局快捷键** — `⌘ + ⇧ + V` 一键唤起/隐藏面板
- 🫧 **悬浮气泡** — 启动后在屏幕右侧显示毛玻璃风格按钮，点击即可打开面板
- 💾 **数据持久化** — 基于 Core Data 存储，重启后历史不丢失
- 🎨 **原生设计** — 淡蓝色配色 + 毛玻璃效果，完全遵循 macOS 设计规范
...
从 [Releases](https://github.com/skynet2000/Clipboard-Pro/releases) 下载 `MClipboard-1.0.0.dmg`，打开后将 MClipboard 拖入 Applications 文件夹即可。
...
首次运行时，系统会弹出提示引导你授予**辅助功能权限**：

> 系统设置 → 隐私与安全性 → 辅助功能 → 添加 MClipboard

这是为了让全局快捷键 `⌘⇧V` 正常工作。不授权也可以正常使用，只是快捷键无法在外部的应用中响应。
